...
export async function POST(request: Request) {
  ...
  // Plan mode runs several full hermes-agent calls sequentially (one per
  // step, each up to HERMES_REQUEST_TIMEOUT_SECONDS=60s on the backend) -
  // the normal 120s ceiling would kill a 3+ step plan mid-execution, so it
  // gets real headroom instead.
