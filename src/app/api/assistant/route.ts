...
export async function POST(request: Request) {
  const body = await request.json().catch(() => null) as { workspaceId?: string; conversationId?: string; content?: string; model?: string; providerId?: string } | null;
  ...
  const upstream = await fetch(assistantGatewayUrl(`/workspaces/${encodeURIComponent(body.workspaceId)}/conversations/${encodeURIComponent(body.conversationId)}/messages`), {
    ...
    body: JSON.stringify({ content: body.content, model: body.model, provider_id: body.providerId }),
    cache: "no-store",
