...
export async function GET(request: Request, { params }: { params: { workspaceId: string } }) {
  ...
  try {
    const response = await fetch(assistantGatewayUrl(`/workspaces/${encodeURIComponent(params.workspaceId)}/projects`), { headers: { authorization }, cache: "no-store" });
    ...
    return NextResponse.json({ error: "VEYAAN project API is unreachable." }, { status: 503 });
  }
}
