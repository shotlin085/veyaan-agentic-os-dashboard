...
export async function POST(request: Request, { params }: { params: { workspaceId: string; providerId: string } }) {
  const authorization = bearerAuthorization(request);
  if (!authorization) return NextResponse.json({ error: "Authentication required." }, { status: 401 });
  try {
    const response = await fetch(
      assistantGatewayUrl(
        `/workspaces/${encodeURIComponent(params.workspaceId)}/providers/${encodeURIComponent(params.providerId)}/refresh-models`,
      ),
