...
export async function DELETE(request: Request, { params }: { params: { workspaceId: string; name: string } }) {
  const authorization = bearerAuthorization(request);
  if (!authorization) return NextResponse.json({ error: "Authentication required." }, { status: 401 });
  try {
    const response = await fetch(
      assistantGatewayUrl(`/workspaces/${encodeURIComponent(params.workspaceId)}/hermes/mcp-servers/${encodeURIComponent(params.name)}`),
