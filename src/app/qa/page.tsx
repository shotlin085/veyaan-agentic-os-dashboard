...
export default function QACentrePage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6 pb-12">
    ...
    <p className="max-w-2xl text-sm leading-6 text-text-muted">Independent test runs and immutable evidence for the selected workspace. Release confidence is derived only from recorded QA results.</p>
    {!workspace ? <Card><EmptyState title="Connect a workspace" description="Sign in with Supabase and select an active workspace to inspect QA evidence." locked /></Card>
      ...
      : <div className="grid gap-4 md:grid-cols-2">{runs.map((run) => <Card key={run.id}><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle>{run.target_ref}</CardTitle><p className="mt-1 text-xs text-text-muted">Project {run.project_id}</p></div><Badge tone={run.verdict === "PASSED" ? "success" : run.verdict === "FAILED" ? "danger" : "warning"}>{run.verdict}</Badge></div></CardHeader><CardContent><div className="flex items-center justify-between text-xs text-text-muted"><span>{run.checks.length} checks</span><span>{run.evidence_refs.length} evidence references</span></div></CardContent></Card>)}</div>}
  </div>;
}
