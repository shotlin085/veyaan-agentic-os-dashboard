    ...
    <div className="mx-auto max-w-5xl space-y-6 p-6 pb-16">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">One tap connects, VEYAAN greets you, then it keeps listening — just talk, pause, or interrupt like a real conversation.</p>
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(18rem,0.8fr)]">
        <Card className="min-h-[30rem] overflow-hidden">
          <CardHeader><CardTitle>Talk to VEYAAN</CardTitle><p className="text-xs text-muted-foreground">{roomName ? `Room ${roomName}` : "No room connected"}</p></CardHeader>
          <CardContent className="flex min-h-[30rem] flex-col items-center justify-center gap-7 text-center">
            <div className={`flex h-44 w-44 items-center justify-center rounded-full border ${micHeld ? "border-foreground bg-foreground/15 shadow-[0_0_80px_rgba(0,240,255,.25)]" : "border-muted-foreground/50 bg-muted-foreground/10"}`}>
              {micHeld ? <Mic className="h-16 w-16 text-foreground" /> : <Volume2 className="h-16 w-16 text-muted-foreground" />}
            </div>
            <div>
              <p className="text-2xl font-semibold text-white">{statusCopy[status]}</p>
              <p className="mt-2 max-w-md text-sm leading-6 text-muted-foreground">{caption}</p>
              {userCaption && <p className="mt-1 max-w-md text-xs leading-5 text-foreground">You: {userCaption}</p>}
            </div>
            <div className="flex flex-wrap justify-center gap-3">
              {!roomName ? (
                ...
                <button
                  type="button"
                  className={`flex h-16 w-16 items-center justify-center rounded-full border ${micHeld ? "border-foreground bg-foreground/15 text-foreground" : "border-status-danger/50 bg-status-danger/10 text-status-danger"}`}
            ...
            <p className="text-xs text-muted-foreground">{roomName ? (micHeld ? "Listening continuously — just speak, no need to press anything." : "Microphone muted. Tap the mic to resume.") : "The browser will ask for microphone permission once you tap Talk to VEYAAN."}</p>
            <div ref={audioRef} className="sr-only" aria-live="polite" />
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle>Live transcript</CardTitle><p className="text-xs text-muted-foreground">Versioned LiveKit events from the worker.</p></CardHeader>
