      ...
      <main className="flex min-w-0 flex-1 flex-col"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-border-subtle px-4 py-3 sm:px-6"><div className="flex min-w-0 items-center gap-3"><Button variant="ghost" size="sm" className="lg:hidden" onClick={() => setNavOpen(true)}><GitFork className="h-4 w-4" />Studio</Button><div className="min-w-0"><p className="truncate text-sm font-semibold text-white">Workspace workflow</p><p className="truncate text-[10px] font-mono text-text-muted">No workflow selected</p></div></div><div className="flex items-center gap-2">{connected ? <Badge tone="neutral"><span className="hidden sm:inline">Connected — no workflows yet</span><span className="sm:hidden">Connected</span></Badge> : <Badge tone="warning"><AlertTriangle className="h-3 w-3" /><span className="hidden sm:inline">Requires workspace access</span><span className="sm:hidden">Locked</span></Badge>}<Button size="sm" disabled={!connected} onClick={() => setShowNewWorkflow(true)}><Plus className="h-3.5 w-3.5" /><span className="hidden sm:inline">New workflow</span></Button><Button variant="secondary" size="sm" className="xl:hidden" aria-label={inspectorOpen ? "Close Inspector" : "Open Inspector"} onClick={() => setInspectorOpen((value) => !value)}><Settings2 className="h-4 w-4" /></Button></div></div>
        ...
        {showNewWorkflow && <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/70 p-4" onClick={() => setShowNewWorkflow(false)}><div className="w-full max-w-md rounded-2xl border border-border-subtle bg-[#0b0f1a] p-5" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="mt-4 space-y-3">
            <div><label className="mb-1 block text-xs text-text-muted">Agent</label><input value={agentDisplayName} onChange={(e) => setAgentDisplayName(e.target.value)} className="w-full rounded-lg border border-border-subtle bg-bg-app p-2.5 text-xs text-text-primary focus:border-accent-cyan focus:outline-none" /></div>
            <div><label className="mb-1 block text-xs text-text-muted">Task</label><textarea value={taskText} onChange={(e) => setTaskText(e.target.value)} rows={4} placeholder="e.g. Build a landing page for a summer sale campaign" className="w-full rounded-lg border border-border-subtle bg-bg-app p-2.5 text-xs text-text-primary focus:border-accent-cyan focus:outline-none" /></div>
          </div>
          ...
          <div className="mt-4 flex justify-end gap-2"><button onClick={() => setShowNewWorkflow(false)} className="rounded-lg border border-border-subtle bg-bg-surface-2 px-4 py-2 text-xs text-text-secondary">Cancel</button><button disabled={creating || !taskText.trim()} onClick={() => void createWorkflow()} className="rounded-lg bg-accent-cyan px-4 py-2 text-xs font-semibold text-bg-app disabled:opacity-40">{creating ? "Creating…" : "Create"}</button></div>
        </div></div>}
